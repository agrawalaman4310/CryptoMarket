...
export default function CoinSell() {
  ...
  useEffect(() => {
    setcurrprise(
      ((`${state.data.current_price}` / 1) * 1).toLocaleString("en-IN", {
  ...
  const getusertransaction_byQuantity = async () => {
    if (Number(Quantity) <= 0) {
      ...
      let object = {
        ...
        Date: new Date(),
        Prise: (`${state.data.current_price}` / 100) * 83,
        type: "Sell",
      };
      ...
      console.log((`${state.data.current_price}` / 100) * 83 * Quantity);
      console.log(currBalance);

      const response = await axios({
        method: "POST",
        url: "http://localhost:5001/transactions/selltransactions",
        data: {
          Quantity: Quantity,
          Amount: (`${state.data.current_price}` / 100) * 83 * Quantity,
          login: login,
    ...
    if (login) {
      if (Number(Amount_for_amount) <= 0) {
        ...
        let object = {
          ...
          CoinName: state.data.name,
          Quantity:
            Amount_for_amount / ((`${state.data.current_price}` / 100) * 83),
  ...
  useEffect(() => {
    setAmount(
      ((`${state.data.current_price}` / 1) * 1 * Quantity).toLocaleString(
        "en-IN",
        {
          maximumFractionDigits: 2,
          style: "currency",
